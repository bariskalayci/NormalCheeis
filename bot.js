...
fs.readdir("./komutlar/", (err, files) => {
  ...
  files.forEach(f => {
    ...
    log(`[SS] Yüklenen komut: ${props.help.name}.`);
...
client.on("guildMemberAdd", async(member) => {
  ...
  if(sunucupaneli) {
    ...
    let toplamuye = member.guild.channels.cache.find(x =>(x.name).startsWith("Toplam Kişi •"))
    let toplamaktif = member.guild.channels.cache.find(x =>(x.name).startsWith("Aktif Görünen •"))
    let botlar = member.guild.channels.cache.find(x =>(x.name).startsWith("Bot Sayısı •"))
    let rekoraktif = member.guild.channels.cache.find(x =>(x.name).startsWith("Aktiflik •"))
...
client.on("guildMemberRemove", async(member) => {
  ...
  if(sunucupaneli) {
    ...
    if(member.guild.members.cache.filter(off => off.presence.status !== 'offline').size > rekoronline) {
      db.set(`panelrekor_${member.guild.id}`, member.guild.members.cache.filter(off => off.presence.cache.status !== 'offline').size)
    }
    try{
      toplamuye.setName(`Toplam Üye • ${member.guild.members.cache.size}`)
      toplamaktif.setName(`Aktif Üye • ${member.guild.members.cache.filter(off => off.presence.status !== 'offline').size}`)
...
client.on("voiceStateUpdate", async(member) => {
  ...
  if(sunucupaneli) {
    ...
    let seste = member.guild.channels.cache.find(x =>(x.name).startsWith("Seste •"))
    ...
    try{
      toplamuye.setName(`Toplam Üye • ${member.guild.members.cache.size}`)
      toplamaktif.setName(`Aktif Üye • ${member.guild.members.cache.filter(off => off.presence.status !== 'offline').size}`)
      botlar.setName(`Botlar • ${member.guild.members.cahe.filter(m => m.user.bot).size}`)
      rekoraktif.setName(`Rekor Aktiflik • ${rekoronline}`)
      seste.setName(`Seste • ${member.guild.members.cache.filter(a => a.voiceChannel).size}`)
   } catch(e) { }
  }
})
